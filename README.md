...
- `HostCatApp`：
  ...
  - 设置页面：采用原生分组卡片布局，提供跟随系统、简体中文和 English 的即时语言切换，并集中承载开机自启动、Helper 状态、注册、审批与刷新操作。
  - 简体中文与英文字符串资源：覆盖菜单栏、编辑/预览、Helper、备份、设置及主要错误反馈；切换语言后窗口与菜单项立即同步刷新，无需重启或悬停触发更新。
